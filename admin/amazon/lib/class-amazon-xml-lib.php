...
class Ced_Amzon_XML_Lib {
	...
	public function fetchMetaValueOfProduct( $product_id, $metaKey, $getopt_data = '' ) {
		...
		if ( $this->isProfileAssignedToProduct ) {
			...
			if ( ! empty( $this->profile_data ) && isset( $this->profile_data[ $metaKey ] ) ) {
				...
				$global_profile_mapping_data     = $global_profile_mapping_data_arr[$seller_id];
	...
	public function fetchAssignedProfileDataOfProduct( $product_id, $getopt_data = '', $profileID = '' ) {
		...
		if ( isset( $profileID ) && ! empty( $profileID ) && '' != $profileID ) {
			...
			if ( is_array( $profile_data ) ) {
				...
				if ( empty( $template_details ) || '' == $template_details ) {
					...
					// $json_data = $temp_profile_data['category_attributes_response'];
					...
					if ( is_array( $amazonProductsDetails ) ) {
						...
						if ( isset( $required_field['Mandantory'] ) ) {
							...
							$template_details['mandantory']               = json_encode( $required_fields );
							$template_details['category_specific_fields'] = json_encode( $categories_specific_fields );
							$template_details['possible_fields_types']    = json_encode( $possible_type );
							$template_details['template_details_info']    = json_encode( $final_template_details );
						}

						$template_details['product_details'] = json_encode( $amazonProductsDetails );
					}
					...
					// update_option( 'ced_profile_' . $template_name . 'template', $template_details );
				...
				//echo "<pre>"; print_r($profile_data); echo "</pre>"; die('>>fetchAssignedProfileDataOfProduct');
		...
		// if(isset($profile_data['browse_node_ids']['0']) && $profile_data['browse_node_ids']['0']!=''){
		// $this->browse_node_ids = $profile_data['browse_node_ids']['0'];
		// }
	...
	*/
	public function getFormatedData( $productId = '', $wooc_product = '', $productType = '', $getopt_data = '' ) {
		...
		if ( isset( $template_details['category_specific_fields'] ) && ! empty( $template_details['category_specific_fields'] ) ) {
			...
			foreach ( $category_all_fields as $test_keys => $field_key ) {
				...
				if ( $validation_fields && isset( $validation_field_details[ $field_key ]['base'] ) && '' != $validation_field_details[ $field_key ]['base'] && isset( $final_product_details[ $field_key ] ) && '' != $final_product_details[ $field_key ] ) {
					$validated_value = '';
					// $validated_value=$this->validating_amazon_accepted_field_value($validation_field_details[$field_key]['base'],$validation_field_details[$field_key],$final_product_details[$field_key]);
					if ( '' != $validated_value ) {
		...
		//echo "<pre>"; print_r($final_product_details); echo "</pre>"; die('>>FinalProductData');
